...
public class RootServiceImpl implements RootService {
    ...
    public List<Root> findAll() {
        List<Root> roots = rootDao.selectAll();
        return roots;
    }
    ...
    public Page<Root> search(Map searchMap) {
        //通用Mapper多条件搜索的标准写法
        Example example = new Example(Root.class);//指定查询的表tb_root
        //初始化分页条件
        int pageNum = 1;
        int pageSize = 2;
        if (searchMap !=null){
        ...
        Page<Root> roots = (Page<Root>) rootDao.selectByExample(example);
        return roots;
    }

    @Override
    public Boolean add(Root root) {
        int row = rootDao.insert(root);
        if(row>0){
          return true;
        }
    ...
    public Boolean update(Root root) {
        int row = rootDao.updateByPrimaryKeySelective(root);
        if(row>0){
            return true;
        }
